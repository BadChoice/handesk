...
class GetTicketsTest extends TestCase
{
    ...
    public function can_get_a_ticket()
    {
        ...
        $ticket->comments()->createMany(
            factory(Comment::class, 5)->make()->transform(function ($comment) {
                return $comment->setAppends([]);
            })->toArray()
        );
    ...
    public function when_getting_a_ticket_only_public_comments_are_returned()
    {
        $ticket = factory(Ticket::class)->create();
        $ticket->comments()->createMany(
            factory(Comment::class, 2)->make()->transform(function ($comment) {
                return $comment->setAppends([]);
            })->toArray()
        );
        $ticket->comments()->createMany(
            factory(Comment::class, 2)->make(["private" => true])->transform(function ($comment) {
                return $comment->setAppends([]);
            })->toArray()
        );
