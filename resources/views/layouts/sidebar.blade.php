<div class="sidebar">
    ...
    <ul>
        ...
        <li><a href="{{route('leads.index')}}">                 {{ trans_choice('lead.lead',2) }}   </a> <span class="label">{{ $repository->all()->count() }}           </span></li>
        <li><a href="{{route('leads.index')}}?mine=true">       {{ trans_choice('lead.mine',2) }}  </a>  <span class="label">{{ $repository->assignedToMe()->count() }}    </span></li>
